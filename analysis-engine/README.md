...
A standalone microservice for analyzing Ethereum wallet addresses and calculating fraud risk scores for the RevrPay payment platform.
...
_Note: Maximum 10 addresses per batch request_
...
5. **AML Compliance (30%)** - _MetaSleuth Integration_
...
export async function checkWalletRisk(walletAddress: string) {
	try {
		const response = await axios.post(
			`${ANALYSIS_ENGINE_URL}/api/risk/analyze`,
			{
				walletAddress,
			}
		);
