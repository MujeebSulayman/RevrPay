...
import { checkWalletRisk } from '../services/riskService.js';
import {
...
export async function walletRiskMiddleware(c: Context, next: Next) {
	...
	if (!walletAddress) {
		console.log(
			'⚠️ No wallet address in request headers - skipping risk check'
		);
		return next();
	}

	console.log(
		`🔍 Risk check middleware triggered for wallet: ${walletAddress}`
	);
	...
	try {
		...
		if (!riskCheck.allowed) {
			console.log(
				`🚫 Blocking wallet ${walletAddress}: ${riskCheck.blockReason}`
			);
			...
			try {
				...
				if (path.includes('/api/pay/session')) {
					...
					amount = 0.1; // $0.10 for one-time access
				} else {
				...
				if (owner_id) {
					...
					console.error(
						'❌ Cannot record blocked transaction: No valid owner_id found'
					);
				}
			} catch (recordError: any) {
				console.error(
					'❌ Failed to record blocked transaction:',
					recordError.message
				);
			...
			return c.json(
				{
					...
					message:
						'This wallet has been flagged for high risk activity. Please contact support if you believe this is an error.',
				},
				403 // Forbidden
			);
		}
		...
		if (riskCheck.riskAnalysis) {
			console.log(
				`✅ Wallet ${walletAddress} allowed: Risk Score ${riskCheck.riskAnalysis.riskScore}/100 (${riskCheck.riskAnalysis.riskLevel})`
			);
			...
			console.log(
				`⚠️ Risk check failed for ${walletAddress}, allowing by default: ${riskCheck.error}`
			);
		}
		...
		console.error(
			`❌ Risk middleware error for ${walletAddress}:`,
			error.message
		);
...
export async function walletRiskMiddlewareEnhanced(c: Context, next: Next) {
	...
	if (!walletAddress) {
		try {
			...
			if (walletAddress) {
				console.log(
					`📝 Found wallet address in request body: ${walletAddress}`
				);
			}
		} catch (e) {
	...
	if (!walletAddress) {
		console.log(
			'⚠️ No wallet address found in headers or body - skipping risk check'
		);
		return next();
	}
	...
	try {
		...
		if (riskCheck.riskAnalysis) {
			console.log(
				`✅ Wallet allowed: ${riskCheck.riskAnalysis.riskScore}/100 (${riskCheck.riskAnalysis.riskLevel})`
			);
			c.set('riskAnalysis', riskCheck.riskAnalysis);
...
export function createWalletRiskMiddleware(threshold: number) {
	return async (c: Context, next: Next) => {
		...
		try {
			...
			if (
				riskCheck.riskAnalysis &&
				riskCheck.riskAnalysis.riskScore >= threshold
			) {
				console.log(
					`🚫 Blocking wallet (custom threshold ${threshold}): ${walletAddress}`
				);
