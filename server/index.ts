...
const networksFromEnv = process.env.NETWORK?.split(',').map((n) =>
	n.trim()
) || ['base-sepolia'];
...
if (!supabaseUrl || !supabaseAnonKey || !supabaseServiceKey) {
	console.error(
		'❌ Please set SUPABASE_URL, SUPABASE_ANON_KEY, and SUPABASE_SERVICE_ROLE_KEY in the .env file'
	);
	process.exit(1);
}
...
app.use('/api/pay/*', async (c, next) => {
	...
	console.log(
		'🔍 Payment endpoint request headers:',
	...
	c.header(
		...
		'Content-Type, Authorization, X-Requested-With, access-control-expose-headers, x-402-payment, x-402-session, x-payment, x-payment-link, X-Payment-Link, x-402-token, x-402-signature, x-402-nonce, x-402-timestamp, x-402-address, x-402-chain-id, x-402-network, x-402-amount, x-402-currency, x-402-facilitator, x-402-version'
	);
		...
		payTo,
		{
			'/api/pay/sepolia/pyusd/session': {
				price: {
					...
					asset: {
						address:
							'0xCaC524BcA292aaade2DF8A05cC58F0a65B1B3bB9' as `0x${string}`,
						decimals: 6,
						eip712: {
							name: 'PayPal USD',
							version: '1',
		...
		payTo,
		{
			'/api/pay/arbitrum-sepolia/pyusd/session': {
				price: {
					...
					asset: {
						address:
							'0x637A1259C6afd7E3AdF63993cA7E58BB438aB1B1' as `0x${string}`,
						decimals: 6,
						eip712: {
							name: 'PayPal USD',
							version: '1',
...
app.get('/api/risk/wallet/:address', async (c) => {
	...
	const ANALYSIS_ENGINE_URL =
		process.env.ANALYSIS_ENGINE_URL || 'http://localhost:3002';

	try {
		console.log(`📡 Forwarding risk analysis request for wallet: ${address}`);

		const response = await fetch(
			`${ANALYSIS_ENGINE_URL}/api/risk/wallet/${address}`
		);
...
app.post('/api/payment-link', async (c) => {
	try {
			...
			return c.json(
				{
					success: false,
					error:
						'Missing required fields: link_name (string), product_name (string), and expiry_date (ISO string)',
				},
				400
			);
		}
			...
			return c.json(
				{
					success: false,
					error:
						"Invalid expiry_date format. Use ISO date string (e.g., '2024-12-31T23:59:59.000Z')",
				},
				400
			);
		}
		...
		const hashInput =
			product.id + Date.now().toString() + Math.random().toString();
...
app.get('/api/transactions', async (c) => {
	try {
		...
		const stats = {
			...
			cancelled:
				allTransactions?.filter((t) => t.status === 'cancelled').length || 0,
			totalAmount:
				allTransactions
